...
# navigate to Ad New User page - Site Administration > Users > Add New User - validate we are on the Add New User page
# populate the new user form fields using Faker library fake random data
# submit a new user form
# validate new user added:
# search for a new user using email - validate new user is found
...
def search_user():
    print('---------------------------SEARCH-------------------------------')
    if locators.moodle_users_new_main_page_url in driver.current_url and driver.title == locators.moodle_users_new_main_page_title:
...
def delete_user():
    ...
    driver.find_element(By.XPATH, f'//td[contains(., "{locators.full_name}")]/..//td/a[contains(@href, "delete={locators.sysid}")]').click()
    sleep(0.25)
